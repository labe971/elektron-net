...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " elektron-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The elektron-tx tool is used for creating and modifying elektron transactions.\n\n"
                "elektron-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded elektron transaction, or with \"-create [commands]\" to create a hex-encoded elektron transaction.\n"
                "\n"
                "Usage: elektron-tx [options] <hex-tx> [commands]\n"
                "or:    elektron-tx [options] -create [commands]\n"
                "\n";
